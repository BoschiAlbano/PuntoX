...
export default function ComboForm({
  ...
  useEffect(() => {
    if (fullProduct) {
      ...
      if (pathname) {
        setOverride(pathname, fullProduct.Descripcion || "Combo");
      }
    } else if (initialData) {
    ...
    setFotoFile(null);
    setFotoPreview("");
  }, [initialData, fullProduct, pathname, setOverride]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto pb-20">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium mt-1">
              {isEdit
                ...
                : "Completa los datos para registrar un nuevo combo"}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="shadow-sm border border-slate-200/60 bg-white">
            <CardBody className="p-6">
              <h3 className="text-base font-semibold text-slate-800 mb-5 flex items-center gap-2">
                <Package size={18} className="text-[#67afc3]" /> Componentes del
                Combo
              </h3>
              ...
              {(formData.ComponentesCombo || []).length === 0 ? (
                ...
                <div className="space-y-3">
                  {(formData.ComponentesCombo || []).map((comp, index) => {
                    const selectedComp = productos?.find(
                      (p: any) => Number(p.Id) === comp.ComponenteId,
                    );
                      ...
                      >
                        <div className="flex-1 w-full">
                          <Autocomplete
                            ...
                            selectedKey={
                              comp.ComponenteId
                                ? comp.ComponenteId.toString()
                                : null
                            }
                            onSelectionChange={(key) => {
                              const newComps = [
                                ...(formData.ComponentesCombo || []),
                              ];
                              newComps[index].ComponenteId = key
                                ? parseInt(key.toString())
                                : 0;

                              // Al cambiar el producto, ajustar la cantidad al mínimo permitido si no cumple
                              const newSelectedComp = productos?.find(
                                (p: any) =>
                                  Number(p.Id) === newComps[index].ComponenteId,
                              );
                              const newIsPeso =
                                newSelectedComp?.TipoVenta === "PESO";
                              if (
                                !newIsPeso &&
