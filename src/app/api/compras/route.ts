...
import { TIPO_COMPROBANTE_COMPRA, TIPO_MOVIMIENTO, TIPO_PAGO } from "@/lib/constants/comprobantes";
...
const createCompraSchema = z.object({
  proveedorId: z.number().int().positive({ message: "Debe seleccionar un proveedor" }),
  detalles: z.array(detalleCompraSchema).min(1, "Debe agregar al menos un producto"),
  formasPago: z.array(
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sucursalId) return NextResponse.json({ error: "Sucursal no encontrada" }, { status: 400 });
    ...
    const proveedor = await prisma.proveedor.findFirst({
      where: { Id: BigInt(data.proveedorId), TenantId: tenantIdBigInt, EstaEliminado: false },
    });
    ...
    const caja = await prisma.caja.findFirst({
      where: { UsuarioAperturaId: usuarioId, UsuarioCierreId: null, EstaEliminado: false },
    });
    if (!caja) return NextResponse.json({ error: "No tienes una caja abierta" }, { status: 400 });

    // Buscar artículos
    const articulosIds = data.detalles.map((d) => BigInt(d.articuloId));
    const articulos = await prisma.articulo.findMany({
      where: { Id: { in: articulosIds }, TenantId: tenantIdBigInt, EstaEliminado: false },
      include: {
        Iva: true,
        Precio: true,
    ...
    if (articulos.length !== data.detalles.length) {
      return NextResponse.json({ error: "Uno o más artículos no fueron encontrados" }, { status: 404 });
    }

    // Calcular totales
    const subtotalCompra = data.detalles.reduce((sum, d) => sum + d.subtotal, 0);
    const totalFormasPago = data.formasPago.reduce((sum, fp) => sum + fp.monto, 0);
    ...
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      for (const pago of data.formasPago) {
        ...
        const existingDetalle = await tx.detalleCaja.findFirst({
          where: { CajaId: caja.Id, TipoPago: pago.tipoPago, TenantId: tenantIdBigInt, EstaEliminado: false },
        });
        if (existingDetalle) {
