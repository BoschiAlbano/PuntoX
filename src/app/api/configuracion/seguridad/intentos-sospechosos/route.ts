...
export async function GET(req: NextRequest) {
  try {
    ...
    const intentosPorIP = await prisma.$queryRawUnsafe<
      Array<{
        IpAddress: string;
        count: bigint;
        ultimoIntento: Date;
    ...
    const intentosUltimaHora = await prisma.$queryRawUnsafe<
      Array<{
        IpAddress: string;
        count: bigint;
      }>
    ...
    const ipsUnicas = await prisma.$queryRawUnsafe<
      Array<{
        count: bigint;
      }>
    >(
    ...
    const ultimosIntentos = await prisma.$queryRawUnsafe<
      Array<{
        Id: bigint;
        FechaIntento: Date;
    ...
    if (sospechosos.some((s) => s.intentos24Horas >= 10)) {
      alertas.push({
        ...
        descripcion:
          "Se detectaron más de 10 intentos fallidos desde una misma dirección IP en las últimas 24 horas.",
        ips: sospechosos
          .filter((s) => s.intentos24Horas >= 10)
          .map((s) => s.ipAddress),
      });
    }
    ...
    if (intentosUltimaHora.length > 0) {
      alertas.push({
        ...
        descripcion:
          "Se detectaron múltiples intentos fallidos en la última hora.",
