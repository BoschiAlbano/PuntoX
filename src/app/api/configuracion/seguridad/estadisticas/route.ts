...
export async function GET() {
  try {
    ...
    const sesionesActivasResult = await prisma.$queryRawUnsafe<Array<{ count: bigint }>>(
    ...
    const dispositivosActivosResult = await prisma.$queryRawUnsafe<Array<{ count: bigint }>>(
      `SELECT COUNT(*) as count FROM "DispositivoConfiable" WHERE "TenantId" = $1 AND "EstaActivo" = true`,
      tenantIdBigInt
    );
    const dispositivosActivos = Number(dispositivosActivosResult[0]?.count || 0);

    // Obtener última actividad
    const ultimaSesionResult = await prisma.$queryRawUnsafe<Array<{ FechaUltimaActividad: Date | null }>>(
    ...
    const intentosFallidosResult = await prisma.$queryRawUnsafe<Array<{ count: bigint }>>(
    ...
    const intentosExitososResult = await prisma.$queryRawUnsafe<Array<{ count: bigint }>>(
    ...
    return NextResponse.json(
      {
        estadisticas: {
          sesionesActivas,
          dispositivosActivos,
          ultimaActividad: ultimaSesion?.FechaUltimaActividad ? ultimaSesion.FechaUltimaActividad.toISOString() : null,
