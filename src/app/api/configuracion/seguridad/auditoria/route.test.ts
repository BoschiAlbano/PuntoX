/**
 * Tests para la API de auditoría de seguridad (GET).
 */
...
describe("GET /api/configuracion/seguridad/auditoria", () => {
  ...
  it("retorna 403 si no tiene permiso CONFIGURACION", async () => {
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/configuracion/seguridad/auditoria"
    );
    const res = await GET(req);
    expect(res.status).toBe(403);
  });

  it("retorna 200 con estadísticas y eventos recientes", async () => {
    ...
    expect(data.eventosRecientes).toBeDefined();
  });
});
