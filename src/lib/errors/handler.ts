...
export function handlePrismaError(error: unknown): AppErrorClass {
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      ...
      case "P2025":
        return createError.notFound("El recurso que intentas actualizar no existe");
      case "P2003":
      ...
      default:
        if (isDatabaseConnectionError(error)) {
          return createError.serviceUnavailable("No se pudo conectar a la base de datos");
        }
        return createError.database("Error en la base de datos", { code: error.code });
    }
  }
...
export function handleError(error: unknown): NextResponse {
  ...
  if (isDatabaseConnectionError(error)) {
    const appError = createError.serviceUnavailable("No se pudo conectar a la base de datos");
    return NextResponse.json(
      {
        error: {
          code: appError.code,
