...
async function main() {
  ...
  if (args.length < 2) {
    console.log(
      "📝 Uso: npx tsx scripts/asignar-sucursal-usuario.ts [usuarioId] [sucursalId]"
    );
    console.log("\nEjemplo: npx tsx scripts/asignar-sucursal-usuario.ts 1 4");
    console.log(
  ...
  console.log(
    `🔗 Asignando sucursal ${sucursalId} al usuario ${usuarioId}...\n`
  );
  ...
  try {
    ...
    if (usuario.TenantId !== sucursal.TenantId) {
      console.error(
        `❌ Error: El usuario pertenece al tenant ${usuario.TenantId} pero la sucursal pertenece al tenant ${sucursal.TenantId}`
      );
      console.error(
        `   Los usuarios solo pueden tener acceso a sucursales de su propio tenant`
      );
      process.exit(1);
    }
    ...
    if (existeAsignacion) {
      console.log(
        `⚠️  El usuario ${nombreUsuario} ya tiene acceso a la sucursal "${sucursal.Nombre}"`
      );
      process.exit(0);
    }
    ...
    console.log(
      `\n💡 Ahora el usuario tiene acceso a ${await prisma.usuarioSucursal.count(
        { where: { UsuarioId: usuarioId } }
      )} sucursal(es)`
    );
    console.log(
      `   Cierra sesión y vuelve a iniciar para ver el selector de sucursal\n`
    );
  } catch (error: any) {
    if (error.code === "P2002") {
      console.error(
        `❌ Ya existe una asignación entre este usuario y esta sucursal`
      );
    } else {
