...
async function main() {
  ...
  try {
    ...
    if (superAdmins.length === 0) {
      ...
      console.log(
        "   2. Asignar ese perfil al usuario en la tabla PerfilUsuario"
      );
      return;
    }
    ...
    superAdmins.forEach((admin, index) => {
      ...
      console.log(
        `   - Perfiles: ${admin.PerfilUsuario.map(
    ...
    console.log(
      "   2. O actualizar el username con el script: scripts/actualizar-username-superadmin.ts"
    );
  } catch (error) {
