...
| 6.5. Manejo de error: certificado no coincide con el entorno | ☑ | Probado en homologación real por el usuario: certificado de Homologación + switch en "Producción" → ARCA rechaza con `ns1:cms.cert.untrusted`. Se agregó `interpretarErrorArca()` (`arca.service.ts`) que traduce ese código a un mensaje claro ("el certificado no es válido para el entorno seleccionado..."), aplicado en `autorizarVoucher` y `getUltimoComprobanteAutorizado`. De paso se corrigió un bug encontrado con la prueba real: el mensaje salía duplicado ("Error al consultar... Error al consultar...") porque se prefijaba dos veces (en `arca.service.ts` y de nuevo en `facturacion.service.ts`). |
